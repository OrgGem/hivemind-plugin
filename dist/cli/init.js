...
import { AUTOMATION_LEVELS, EXPERT_LEVELS, GOVERNANCE_MODES, LANGUAGES, OUTPUT_STYLES, PROFILE_PRESETS, createConfig, isValidGovernanceMode, isValidLanguage, isValidExpertLevel, isValidOutputStyle, isValidProfilePreset, isCoachAutomation, normalizeAutomationInput, normalizeAutomationLabel, } from "../schemas/config.js";
...
import { auditHiveFiverAssets, seedHiveFiverOnboardingTasks, } from "../lib/hivefiver-integration.js";
 ...
 */
function generateHiveMindAgentsSection() {
    return `
 ...
 */
export function injectAgentsDocs(directory, silent) {
 ...
 */
function applyProfilePreset(profileKey, options) {
 ...
 */
async function printInitSuccess(directory, config, sessionId, state, silent) {
    if (silent)
        return;
...
function logHiveFiverAuditResult(audit, silent) {
    if (silent)
        return;
    const missingRootCount = audit.rootMissing.commands.length + audit.rootMissing.skills.length + audit.rootMissing.workflows.length;
    const missingOpencodeCount = audit.opencodeMissing.commands.length + audit.opencodeMissing.skills.length + audit.opencodeMissing.workflows.length;
...
export async function initProject(directory, options = {}) {
    ...
    if (options.profile && isValidProfilePreset(options.profile)) {
        ...
        if (!options.silent) {
            ...
            if (config.agent_behavior.constraints.require_code_review)
                log("  ✓ Code review required");
            if (config.agent_behavior.constraints.enforce_tdd)
                log("  ✓ TDD enforced");
            log("");
        }
